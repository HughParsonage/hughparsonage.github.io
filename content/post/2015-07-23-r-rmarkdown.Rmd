...
The ABS publishes shapefiles of statistical areas, but doesn't provide an API to access them.
Although an API is typically associated with frequently changing, small files, the lack of API creates an extra step in this common task. 
PSMA does publish shapefiles, and provide an API, but they are separated into states (reflecting the jurisdictions' 'ownership' of the files).
The API also has a nonstandard licence, which purports to allow free use, but contains a caveat that, again, complicates the automated production.[^*]

[^*]: The caveat, ironically, is an attempt to prevent junk mail. 
...
The `ASGS` package contains the shapefiles from the ABS in package form for convenience.
